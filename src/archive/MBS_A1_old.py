...
class MBS_A1:
    ...
    def _extract_measurement_conditions_from_spectaro_data(self):
        ...
        if "[SpecTaroscoPy — PES image]" in self.l or "[Spectrum Analyzer]" in self.l:
            ...
            self.actscan, _ = r(self.l, "Total Actual Scans", initial_value=self.actscan)
        else:
    ...
    def _handle_spectrum_analyzer_output(self, z_):
        ...
        if self.x_label == "Kinetic Energy (eV)" or self.x_label=='X':
            self.x = copy.deepcopy(z_[0])
            z_ = copy.deepcopy(z_[1:])
        elif self.x_label == "Binding Energy (eV)":
    ...
    def _normalize_raw_z_data(self, z_):
        if self.x_label == "Binding Energy (eV)":
            self.EF = copy.deepcopy(z_[0])
        elif self.x_label == "Kinetic Energy (eV)":
            self.x = copy.deepcopy(z_[0])
        elif self.x_label=='X':
            self.x = copy.deepcopy(z_[0])
    ...
    def save_data(self, filename, data_type, axis):
        ...
        savefile_path = directory_path+f"/{foldername}/{filename}"
        ...
        if axis=="Ek":
            ...
            self.write_data(savefile_path, filename, data_type, axis)
            self.write_data(savefile_path, filename, data_type, "EF")
        elif axis=='Y':
            self.write_data(savefile_path, filename, data_type, axis)

    def write_data(self, savefile_path, filename, data_type, axis):
        def write():
            # DATA: 以降の行を取得
            try:
                index=self.l.index('DATA:')
            ...
            if data_type=='image':
                ...
                for x_value, z_values in zip(x, z.T):  # zは転置して対応
                    rewritten_data.append(f"{x_value:.10f}\t" + "\t".join(f"{z:.10f}" for z in z_values))            
                # 元のファイルのヘッダー部分と書き換えたデータ部分を結合
                output_lines = self.l[:data_index-2] + added_header + rewritten_data

            elif data_type=='edc' or data_type=='ydc':
                # 新しいデータのリストを作成
                edc_data = []
                label_=filename.replace(".txt","")
                if data_type=='ydc':
                    ...
                    # label情報
                    if self.y_label=="Position (mm)":
                        x_label=f"posi_{label_}"
                    elif self.y_label=="Angle (Degrees)":
                        x_label=f"deg_{label_}"
                    elif self.y_label==r"$k_{//} \ \mathrm{(\AA)}$":
                        x_label=f"kh_{label_}"
                else:
                    z=self.z_edc
                    ...
                    if axis=="Ek":
                        ...
                        for i in range(len(self.x)):
                            x.append(f"{self.x_offseted[i]}\t{self.EF_offseted[i]}")
                        x_label=f"x_{label_}\tEF_{label_}"
                    elif axis=="VL":
                        # x=self.VL
                        x=[]
                        for i in range(len(self.VL)):
                            x.append(f"{self.x_offseted[i]}\t{self.EF_offseted[i]}\t{self.VL_offseted[i]}")      
                        x_label=f"x_{label_}\tEF_{label_}\tVL_{label_}"

                # Z offsetかけるとそれをラベルに追加する。
                if z_bg is not None:
                    # if data_type=='ydc':
                        # if self.y_label=="Position (mm)":
                    added_header.append(f"{x_label}\t{label_}\t{label_}_bg")
                    # else: # EDC
                    #     added_header.append(f"{x_label}\t{label_}\t{label_}_bg")
                else:
                    # if data_type=='ydc':
                    #     if self.y_label=='Position (mm)':
                    #         added_header.append(f"{x_label}\t{label_}")
                    # else: # EDC
                    added_header.append(f"{x_label}\t{label_}")
                ...
                if z_bg is not None: # offsetかけた場合
                    for i in range(len(x)): 
                        if z[i]==0:
                            if z_bg[i]==0:
                                edc_data.append(f"{x[i]}\tnan\tnan")
                            else:
                                edc_data.append(f"{x[i]}\tnan\t{z_bg[i]}")
                        else:
                            edc_data.append(f"{x[i]}\t{z[i]}\t{z_bg[i]}")           
                else: # offsetかけてない場合
                    for i in range(len(x)): 
                        if z[i]==0:
                            edc_data.append(f"{x[i]}\tnan")
                        else:
                            edc_data.append(f"{x[i]}\t{z[i]}")


                # 元のファイルのヘッダー部分と書き換えたデータ部分を結合
                output_lines = self.l[:data_index-2] + added_header + edc_data
                ...
                # label
                y_legend_lst = [f"{x_label}"]
                for i in range(len(self.y_slice_center_edcs)):
                    y_legend_lst.append(f"{label_}_{np.round(self.y_slice_center_edcs[i], 3)}")
                # y_legend_lstを適切にフォーマットして文字列に変換
                added_header.append("\t".join(y_legend_lst))
